...
if (typeof describe === "function") {
  describe("extractCoordinates", () => {
    it("extracts coordinates from query parameters", () => {
      const url = "https://example.com?lat=37.44192&lon=-122.14612&zoom=14.02&pitch=9.6&bearing=25";
      const result = extractCoordinates(url);
      const expected = { lat: 37.44192, lon: -122.14612, zoom: 14.02, pitch: 9.6, bearing: 25 };
      expect(result).toEqual(expected);
    });
    it("extracts coordinates from hash with 4 segments", () => {
      const url = "https://example.com/#14.02/37.44192/-122.14612/25";
      const result = extractCoordinates(url);
      const expected = { lat: 37.44192, lon: -122.14612, zoom: 14.02, pitch: 0, bearing: 25 };
      expect(result).toEqual(expected);
    });
    it("extracts coordinates from hash with 5 segments", () => {
      const url = "https://example.com/#14.02/37.44192/-122.14612/25/9.6";
      const result = extractCoordinates(url);
      const expected = { lat: 37.44192, lon: -122.14612, zoom: 14.02, pitch: 9.6, bearing: 25 };
      expect(result).toEqual(expected);
    });
    it("extracts coordinates from path with '/@'", () => {
      const url = "https://www.google.com/maps/@37.802109,-122.4699613,1453a,35y,347.04h,41t/data=!3m1!1e3?entry=ttu";
      const result = extractCoordinates(url);
      // Ожидаемые значения могут отличаться, если нужны точные параметры, но базовая проверка:
  ...
  describe("parseCliString", () => {
    it("parses CLI string correctly", () => {
      const cliString = "--lon -122.14612 --lat 37.44192 --zoom 14.02 --bearing 25 --pitch 9.6";
      const result = parseCliString(cliString);
      expect(result).toEqual({ lon: "-122.14612", lat: "37.44192", zoom: "14.02", bearing: "25", pitch: "9.6" });
    });
  });
}

module.exports = {
  updateActiveTabUrlWithCoordinates,
  extractCoordinates,
  parseCliString,
  displayCoordinates
};